...
void BitCoinExchange::_parseDbFile(const std::string& dbFilePath) {
  ...
  if (!file.good())
    throw BitCoinExchangeException("File does not exist or cannot be opened");

  std::string line;
  std::getline(file, line);  // read and discard the headerline

  while (std::getline(file, line)) {
    /* I save the line I read with getline to isstream so I can read the line
     * again to seperate the date, and the exrate */
    ...
    if (std::getline(ss, date, ',') && std::getline(ss, rateStr)) {
      double rate;
      std::istringstream convertStream(rateStr);

      /* If I can convert the rateStr to rate (double) using istringstream push
       * it to the map */
      if (!(convertStream >> rate))
        throw BitCoinExchangeException("The string was not a valid number");
